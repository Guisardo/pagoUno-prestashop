...
class PagounoValidationModuleFrontController extends ModuleFrontController
{
    ...
    public function postProcess()
    {
        ...
        if (Tools::getValue("pagouno_cuotas") == 'no' || Tools::getValue("pagouno_cuotas") === false) {
                            ...
                            case 200:
                                if ($body['data']['success']) {
                                    try {
                                        ...
                                        if (!$validationResult) {
                                            PrestaShopLogger::addLog(json_encode(array(
                                                (int) $this->context->cart->id,
                                                $pustatus['status_aprobada'],
                                                $this->context->cart->getOrderTotal(true, Cart::BOTH),
                                                $metodo_de_pago,
                                                ...
                                                false,
                                                $customer->secure_key
                                            )), 2, null, 'pagouno', $this->context->cart->id);
                                            PrestaShopLogger::addLog(json_encode($body), 2, null, 'pagouno', $this->context->cart->id); 
                                        }
...
//                                        );
                                        ...
                                        PrestaShopLogger::addLog(json_encode($e), 3, null, 'pagouno', $this->context->cart->id);
                                    }
                                } else {
